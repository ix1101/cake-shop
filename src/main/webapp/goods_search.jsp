        ...
        <div>
            <div style='text-align:center;'>
                <a class='btn btn-info'
                   ...
                   <c:if test="${p.pageNo!=1 }">href="${pageContext.request.contextPath }/goods_search?pageNo=1&keyword=${keyword}"</c:if>>首页</a>
                <a class='btn btn-info'
                   <c:if test="${p.pageNo==1 }">disabled</c:if>
                   <c:if test="${p.pageNo!=1 }">href="${pageContext.request.contextPath }/goods_search?pageNo=${p.pageNo-1}&keyword=${keyword}"</c:if>>上一页</a>
                ...
                <a class='btn btn-info'
                   ...
                   <c:if test="${p.pageNo!=p.totalPage }">href="${pageContext.request.contextPath }/goods_search?pageNo=${p.pageNo+1}&keyword=${keyword}"</c:if>>下一页</a>
                <a class='btn btn-info'
                   <c:if test="${p.totalPage==0 || p.pageNo==p.totalPage }">disabled</c:if>
                   <c:if test="${p.pageNo!=p.totalPage }">href="${pageContext.request.contextPath }/goods_search?pageNo=${p.totalPage}&keyword=${keyword}"</c:if>>尾页</a>
