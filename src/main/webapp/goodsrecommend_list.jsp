        ...
        <div>
            <div style='text-align:center;'>
                <a class='btn btn-info'
                   ...
                   <c:if test="${p.pageNo!=1 }">href="${pageContext.request.contextPath }/goodsrecommend_list?pageNo=1&type=${t}"</c:if>>首页</a>
                <a class='btn btn-info'
                   <c:if test="${p.pageNo==1 }">disabled</c:if>
                   <c:if test="${p.pageNo!=1 }">href="${pageContext.request.contextPath }/goodsrecommend_list?pageNo=${p.pageNo-1}&type=${t}"</c:if>>上一页</a>
                ...
                <a class='btn btn-info'
                   ...
                   <c:if test="${p.pageNo!=p.totalPage }">href="${pageContext.request.contextPath }/goodsrecommend_list?pageNo=${p.pageNo+1}&type=${t}"</c:if>>下一页</a>
                <a class='btn btn-info'
                   <c:if test="${p.totalPage==0 || p.pageNo==p.totalPage }">disabled</c:if>
                   <c:if test="${p.pageNo!=p.totalPage }">href="${pageContext.request.contextPath }/goodsrecommend_list?pageNo=${p.totalPage}&type=${t}"</c:if>>尾页</a>
